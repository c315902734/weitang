...
class topic_cateModel extends baseModel
{
    ...
    {
        $res = M('topic_cate_tag')->field('tag_id')->where(array('cate_id' => $cate_id))->select();
        $ids = array();
        foreach ($res as $tag) {
            $ids[] = $tag['tag_id'];
        }
    ...
    {
        //分类数据
        if (false === $cate_data = F('topic_cate_data')) {
    ...
    public function relate_cache()
    {
        ...
        F('topic_cate_relate', $cate_relate);
        return $cate_relate;
    }
    ...
    {
        F('topic_cate_data', NULL);
        F('topic_cate_list', NULL);
        F('topic_cate_relate', NULL);
    }
