...
class Tree {
	...
	*/
	public function get_parent($myid){
		...
		$pid = $this->arr[$myid][$this->ppid];
		$pid = $this->arr[$pid][$this->ppid];
		if(is_array($this->arr)){
			foreach($this->arr as $id => $a){
				if($a[$this->ppid] == $pid) $newarr[$id] = $a;
			}
		}
		return $newarr;
	}
	...
	*/
	public function get_child($myid){
		$a = $newarr = array();
        // 初始化后，成员arr 里面已经有所有的菜单数组了
		if(is_array($this->arr)){
			foreach($this->arr as $id => $a){
                // if( $a['pid'] == $myid ) $newarr[$id] =$a;
                // 即顶级菜单的父id等于 0 ，则将其赋给新数组，此处$a为多维数组（因为不清楚该顶级菜单下有多少子级菜单）
				if($a[$this->ppid] == $myid) $newarr[$id] = $a;
			}
		}
	...
	*/
	public function get_tree($myid, $str, $sid = 0, $adds = '', $str_group = ''){
		$number=1;
        // 获得这个$myid下的所有子级数组
        // 返回顶级菜单的父id等于0 ，则将其赋给新数组，此处$a（$child）为多维数组（因为不清楚该顶级菜单下有多少子级菜单）
		$child = $this->get_child($myid);
        /*$myid =0;

        */
		if(is_array($child)){
		    $total = count($child); //同一父级id=0下有几个二级分类
			foreach($child as $id=>$value){
				$j=$k='';
				if($number==$total){
                    // 比如菜单中的“商家管理”模块，其二级菜单就一个“商家管理”（即$number=$total==1），所以$j = '└';
                    // 树状结构样式为：商家管理
                    //             └─ 商家管理
					$j .= $this->icon[2];  // $icon = array('│','├','└');
				}else{
                    // $j = '├';  $k ='';
					$j .= $this->icon[1];
					$k = $adds ? $this->icon[0] : '';
				}
				$spacer = $adds ? $adds.$j : '';  // $spacer = '';
	...
	*/
	public function get_tree_multi($myid, $str, $sid = 0, $adds = ''){
		...
		$child = $this->get_child($myid);
		if(is_array($child)){
		    $total = count($child);  //同一父级id下有几个二级分类
