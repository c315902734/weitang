...
class Arr
{
    ...
    public static function fetch($key_str, $arr)
    {
        $result = array();
    ...
    public static function multisort($field, &$list, $order = SORT_DESC)
    {
        $volume = array();
        foreach ($list as $val) {
            $volume[] = $val[$field];
        }
        array_multisort($volume, $order, $list);
    }
    ...
    public static function is_num_key($arr)
    {
        $keys = array_keys($arr);
        return $keys === array_keys($keys);
    }

    public static function pick($arr, $fields)
    {
        if (empty($fields)) return $arr;
        ...
        $result = array();
        //var_dump(Arr::is_num_key($arr));exit();
        if (Arr::is_num_key($arr)) {
            foreach ($arr as $key => $val) {
                if (count($fields) == 1) {
                    $result[] = $val[$fields[0]];
                } else {
                    $result[$key] = Arr::pick($val, $fields);
                }
            }
        } else {
            foreach ($arr as $key => $val) {
    ...
    public static function omit($arr, $fields)
    {
        $keys = array_keys($arr);
        $is_num_index = array_keys($keys) == array_values($keys);
        ...
        $result = array();
        foreach ($arr as $key => $val) {
            if (!in_array($is_num_index ? $val : $key, $fields)) {
                $result[$key] = $val;
            }
        }
        return $result;
