...
class oauth {
    ...
    public function callbackLogin($request_args) {
        ...
        if ($bind_user) {
            ...
            //判断是否填写手机号
            $where['id'] = $bind_user['uid'];
            $where['tele'] = array('lt', 13000000000);
            $tele_count = D('user')->where($where)->count();
            
            if($tele_count){
                return U('user/tele_bind');
            } else {
                return U('user/index');
            }
            //return U('user/index');
            ...
            if (M('user')->where(array('username' => $user['keyname']))->count()) {
                $user['ins_user_name'] = $user['keyname'] . '_' . mt_rand(99, 9999);
            } else {
                $user['ins_user_name'] = $user['keyname'];
            }
            $user['ins_user_name'] = urlencode($user['ins_user_name']);
            $user['keyname'] = urlencode($user['keyname']);
            if ($user['keyavatar_big']) {
                //下载原始头像到本地临时储存  用日期文件夹分类  方便清理
                $user['temp_avatar'] = '';
                $avatar_temp_root = C('ins_attach_path') . 'avatar/temp/';
    ...
    public function callbackBind($request_args) {
        ...
        $ins_user = session('user_info');
        $user = $this->_om->getUserInfo($request_args);
        $bind_user = $this->_checkBind($this->_type, $user['keyid']);
        if ($bind_user['uid'] && $bind_user['uid'] != $ins_user['id']) {
            die('此帐号已经绑定过本站');
        }
        $user['ins_uid'] = $ins_user['id'];
        $this->bindUser($user);
        return U('user/bind');
    }
