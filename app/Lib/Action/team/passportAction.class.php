...
class passportAction extends teamAction
{
    public function login()
    {
        ...
        if (IS_POST) {
            ...
            //$ret_url  = $this->_post('ret_url', 'trim', U('user/index'));
			...
			/* 是否存在团队级别 */
			if(D('user')->where(array('id'=>$uid))->getField('tgroup') < 1){
                $this->ajaxResultError('该帐号级别不足!');
			}
            //登陆
